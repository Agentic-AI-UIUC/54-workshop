#!/usr/bin/env bash
# Self-check for guard.sh. Run: bash .cursor/hooks/test.sh
cd "$(dirname "$0")/../.."
pass=0; fail=0
check() {
  got=$(printf '%s' "$2" | bash .cursor/hooks/guard.sh | python3 -c 'import sys,json;print(json.load(sys.stdin)["permission"])')
